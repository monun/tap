...
class FakeEntityImpl internal constructor(
        ...
        fun update() {
            ...
            for (slot in EquipmentSlot.values()) {
                ...
                if ((old == null && current.type != Material.AIR)
                    ...
                    trackers.sendServerPacketAll(PacketSupport.INSTANCE.entityEquipment(armorStand.entityId, mapOf(slot to current)))
                }
            }
        }
    }
    ...
    internal fun update() {
        ...
        if (updateMeta) {
            ...
            trackers.sendServerPacketAll(PacketSupport.INSTANCE.entityMetadata(bukkitEntity))
        }
        ...
        effects.let { effects ->
            while (effects.isNotEmpty()) {
                trackers.sendServerPacketAll(PacketSupport.INSTANCE.entityStatus(bukkitEntity.entityId, effects.remove()))
            }
        }
    }
    ...
    private fun updateLocation(): MoveResult {
        ...
        val result =
            if (updateTeleport || from.world !== to.world || deltaX < -32768L || deltaX > 32767L || deltaY < -32768L || deltaY > 32767L || deltaZ < -32768L || deltaZ > 32767L) { // Teleport
                ...
                trackers.sendServerPacketAll(PacketSupport.INSTANCE.entityTeleport(bukkitEntity, to))
                MoveResult.TELEPORT
            } else { //Relative move
                val yaw = to.yaw
                val pitch = to.pitch

                val packet = PacketSupport.INSTANCE.relEntityMoveLook(
                    bukkitEntity.entityId,
                    deltaX.toShort(),
                    deltaY.toShort(),
                    deltaZ.toShort(),
    ...
    internal fun despawn() {
        trackers.sendServerPacketAll(PacketSupport.INSTANCE.entityDestroy((bukkitEntity.entityId)))
    }
